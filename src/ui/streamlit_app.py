...
class PortfolioTrackerUI:
    ...
    def plot_portfolio_allocation(self, positions):
        ...
        # Create pie chart
        fig = px.pie(
            values=values, names=symbols, title="Portfolio Allocation by Position"
        )
        ...
        with st.form("add_transaction"):
            ...
            with col3:
                ...
                days_ago = st.number_input(
                    "Days Ago", min_value=0, max_value=365, value=0
                )
    ...
    def render_analytics(self, portfolio_manager, metrics_calculator):
        ...
        if len(snapshots) < 2:
            st.warning(
                f"Insufficient data for {days}-day analysis. Please add more historical data."
            )
            return
